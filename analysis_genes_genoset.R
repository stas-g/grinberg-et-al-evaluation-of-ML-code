#---------------------------------------------------------------------------------------------------------------------------------------#
#                          CODE FOR ANALYSIS ON AN ALTERNATIVE DATA SET: GENO.GENES (4383 BINARY ATTRIBUTES)                       #
#---------------------------------------------------------------------------------------------------------------------------------------#

geno.genes <- readRDS('geno_genes.rds')
...
rf.cvpred <- foreach(i = 1 : ncol(pheno2), .options.RNG = 100, .combine = cbind) %dorng% {
	pred <- forest.xpred(geno.genes, pheno2[, i], yfolds, err = FALSE, importance = TRUE, ntree = 700, do.trace = 100)
...
gbm.cvpred <- foreach(i = 1 : ncol(pheno2), .options.RNG = 100, .combine = cbind) %dorng% {
	...
	pred <- gbm.xpred(geno.genes, pheno2[, i], yfolds, err = FALSE, distribution = 'gaussian', n.trees = 1000, interaction.depth = 5, n.minobsinnode = 10, keep.data = FALSE, verbose = TRUE, shrinkage = 0.01, bag.fraction = 0.5, nTrain = nTrain_)
	pred
	}
...
lasso.cvpred <- foreach(i = 1 : ncol(pheno2), .options.RNG = 100, .combine = cbind) %dorng% {
				pred <- lasso.xpred(geno.genes, pheno2[, i], yfolds, err = FALSE, family = 'gaussian', alpha = 1, standardize = FALSE, nfolds = 10, parallel = F)
...
ridge.cvpred <- foreach(i = 1 : ncol(pheno2), .options.RNG = 100, .combine = cbind) %dorng% {
				pred <- ridge.xpred(geno.genes, pheno2[, i], yfolds, err = FALSE, family = 'gaussian', alpha = 0, standardize = FALSE, nfolds = 10, parallel = F)
				pred
			}
...
genes_rsq <- cbind(rf = rf_rsq, gbm = gbm_rsq, lasso = lasso_rsq, ridge = ridge_rsq) 
saveRDS(genes_rsq, file = 'genes_rsq.rds')
